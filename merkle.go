...
func newMerkleTree(transactions []transaction_I) *merkleNode_T {
	...
	for {
		...
		if len(level) % 2 != 0 {
			newLevel = make([]*merkleNode_T, len(level) / 2, len(level) / 2)
		} else {
			newLevel = make([]*merkleNode_T, len(level) / 2 + 1, len(level) / 2 + 1)
		}

		for k, _ := range newLevel {
			fmt.Println("kkkkkkkkkkkkkk:", k)
...
func newMerkleTree(transactions []transaction_I) *merkleNode_T {
	...
	for k, hash := range hashes {
		level[k] = &merkleNode_T { nil, nil, hash }
	}
